...
class HierarchicalForecaster:
    ...
    def create_features(
        ...
        df["trend"] = df.groupby(state_col).cumcount()
    ...
    def train(
        ...
        with perf_logger.timer("hierarchical_forecasting_training"):
            ...
            for fold, (train_idx, val_idx) in enumerate(tscv.split(X)):
                ...
                logger.info(
                    f"Fold {fold + 1}/{n_splits}", extra={"rmse": rmse, "mae": mae}
                )

            # Train final model on all data
            train_data = lgb.Dataset(X, label=y)
            self.model = lgb.train(self.lgb_params, train_data, num_boost_round=1000)
    ...
    def forecast(
        ...
        with perf_logger.timer("hierarchical_forecasting_inference"):
            ...
            logger.info(
                ...
                extra={"n_states": len(states), "horizon": self.horizon},
            )
    ...
    def _forecast_state(
        ...
        future_dates = pd.date_range(
            start=last_date + pd.DateOffset(months=1), periods=self.horizon, freq="MS"
        )
        ...
        for future_date in future_dates:
            ...
            forecast_rows.append(
                {state_col: state, date_col: future_date, "forecast": prediction}
            )
    ...
    def _reconcile_bottom_up(
        self, forecast_df: pd.DataFrame, date_col: str
    ) -> pd.DataFrame:
        """
        ...
        national_forecast = (
            forecast_df.groupby(date_col)["forecast"].sum().reset_index()
        )
        ...
        reconciled_df = pd.concat([forecast_df, national_forecast], ignore_index=True)
...
def compare_with_sarima(
    ...
    hier_rmse = np.sqrt(
        np.mean((actual["value"] - hierarchical_forecast["forecast"]) ** 2)
    )
