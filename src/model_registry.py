...
class ModelRegistry:
    ...
    def register_sarima_model(
        ...
        with mlflow.start_run(run_name=f"SARIMA_{state}"):
            # Log parameters
            p, d, q = order
    ...
    def register_clustering_model(
        ...
        with mlflow.start_run(run_name=f"GMM_K{n_clusters}"):
            # Log parameters
            mlflow.log_params(
                {
            ...
            if hasattr(model, "means_"):
                centroids_df = pd.DataFrame(model.means_, columns=feature_names)
    ...
    def register_priority_model(
        ...
        with mlflow.start_run(run_name="PriorityScoring"):
            # Log parameters
            mlflow.log_params(
                {
            ...
            logger.info(
                "Registered priority scoring model",
                extra={
                    "model_name": model_name,
                    "version": model_version,
                    "r2": metrics.get("r2"),
