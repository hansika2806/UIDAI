...
class SyntheticControl:
    ...
    def fit(
        ...
        actual_outcome = df[df[unit_col] == self.treated_unit].set_index(date_col)[
            self.outcome_col
        ]
        self.treatment_effect = actual_outcome - self.synthetic_outcome

        # Calculate fit metrics
        pre_treatment_effect = self.treatment_effect[
            self.treatment_effect.index < self.intervention_date
        ]
        ...
        logger.info(
            ...
            extra={
                ...
                "top_3_weights": sorted(
                    results["weights"].items(), key=lambda x: x[1], reverse=True
                )[:3],
            },
        )
    ...
    def _fit_optimization(
        ...
        control_matrix = control_data.pivot(
            index=date_col, columns=unit_col, values=self.outcome_col
        )[self.control_units]
    ...
    def _calculate_synthetic_outcome(
        self, df: pd.DataFrame, unit_col: str, date_col: str
    ) -> pd.Series:
        """
    ...
    def estimate_treatment_effect(self, post_period_only: bool = True) -> Dict:
        """
    ...
    def placebo_test(
        ...
        for i in range(n_placebos):
            ...
            try:
                ...
                placebo_result = placebo_sc.estimate_treatment_effect(
...
def run_synthetic_control_analysis(
    ...
    control_units = [
        u for u in all_units if u != treated_unit and u not in exclude_units
    ]
