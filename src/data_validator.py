...
class UidaiDataValidator:
    ...
    def validate_enrolment_data(self, df: pd.DataFrame) -> Dict:
        ...
        results["has_aadhaar_generated"] = ge_df.expect_column_to_exist(
            "aadhaar_generated"
        )
        ...
        if "aadhaar_generated" in df.columns:
            results["aadhaar_generated_non_negative"] = (
        ...
        # Null validation
        results["registrar_not_null"] = ge_df.expect_column_values_to_not_be_null(
            "registrar"
        )
    ...
    def validate_update_data(self, df: pd.DataFrame, update_type: str) -> Dict:
        ...
        if "registrar" in df.columns and "date" in df.columns:
            ...
            if duplicate_count > 0:
                logger.warning(
                    f"Found {duplicate_count} duplicate state-date combinations"
                )
    ...
    def validate_state_master(self, df: pd.DataFrame) -> Dict:
        ...
        for col in critical_cols:
            if col in df.columns:
                results[f"{col}_not_null"] = ge_df.expect_column_values_to_not_be_null(
                    col
                )
