...
class DriftMonitor:
    ...
    def psi(
        ...
        if drift_detected:
            logger.warning(
                f"Feature drift detected (PSI): {feature_name}",
                extra=result
            )
    ...
    def js_divergence(
        ...
        if drift_detected:
            logger.warning(
                f"Feature drift detected (JS): {feature_name}",
                extra=result
            )
    ...
    def chi2_test(
        ...
        all_categories = np.unique(np.concatenate([reference, production]))
        ...
        if drift_detected:
            logger.warning(
                f"Categorical drift detected: {feature_name}",
                extra=result
            )
    ...
    def monitor_dataframe(
        ...
        drifted_features = [
            col for col, res in results["features"].items()
            if res.get("drift_detected", False) or 
        ...
        results["drift_percentage"] = len(drifted_features) / len(reference_df.columns) * 100
