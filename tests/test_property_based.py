...
class TestForecastingProperties:
    ...
    @given(st.lists(st.floats(min_value=1.0, max_value=1000.0), min_size=3, max_size=50))
...
class TestAggregationProperties:
    ...
    @given(
        data_frames([
            column('value', dtype=float, elements=st.floats(min_value=0, max_value=1000)),
    ...
    @given(
        st.lists(st.floats(min_value=0, max_value=1000), min_size=5, max_size=100)
    )
...
class TestTemporalProperties:
    ...
    @given(
        st.lists(st.integers(min_value=1, max_value=12), min_size=12, max_size=36)
    )
