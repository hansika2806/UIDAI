...
def render_causal_panel() -> None:
    ...
    selected_state = st.selectbox(
        "Select State to Inspect Causal Dynamics:", all_states
    )
    ...
    with d_cols[1]:
        ...
        cusum_max = (
            state_cusum.get("cusum_max", float("nan"))
            if hasattr(state_cusum, "get")
            else state_cusum["cusum_max"]
        )
        crit_val = (
            state_cusum.get("critical_value", float("nan"))
            if hasattr(state_cusum, "get")
            else state_cusum.get("critical_value", float("nan"))
        )
        break_det = (
            bool(state_cusum.get("break_detected", False))
            if hasattr(state_cusum, "get")
            else bool(state_cusum["break_detected"])
        )
        ...
        if break_det:
            break_month = (
                state_cusum.get("break_month", "Unknown")
                if hasattr(state_cusum, "get")
                else state_cusum["break_month"]
            )
            st.success(
