...
def render_executive_overview(
    ...
    with right:
        st.plotly_chart(build_category_mix_chart(state_master), use_container_width=True)
...
def render_governance_diagnostics(
    ...
    st.plotly_chart(build_rank_persistence_chart(rank_df, compare_mode), use_container_width=True)
...
def render_lifecycle_operations(
    ...
    with right:
        st.plotly_chart(build_volatility_bar_chart(state_master), use_container_width=True)
...
def render_anomalies_risk(anomalies: pd.DataFrame) -> None:
    ...
    with right:
        ...
        if pressure_mismatch.empty:
            ...
            st.plotly_chart(build_mismatch_queue_chart(pressure_mismatch), use_container_width=True)
...
def render_state_drilldown(
    ...
    st.plotly_chart(build_state_ratio_trend_chart(state_series, selected_state), use_container_width=True)
