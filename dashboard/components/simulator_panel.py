...
LEVERS = {
    "enrollment_drive": {
        ...
        "min": 0, "max": 100, "default": 0, "step": 5, "unit": "%",
    },
    "fee_waiver": {
    ...
    "seasonal_uniformity": {
        ...
        "min": 0, "max": 100, "default": 0, "step": 10, "unit": "%",
    },
}
...
def compute_indicator_deltas(levers: Dict[str, float]) -> Dict[str, float]:
    ...
    delta_AMI = (
        _diminishing(ed, max_effect=0.12, k=2.5)  # enrollment broadening
        + _diminishing(fw, max_effect=0.04, k=2.0)  # fee waiver encourages updates
    )

    # UPI: driven by mobile linkage (primary) + fee waivers (secondary)
    delta_UPI = (
        _diminishing(ml, max_effect=0.18, k=3.0)  # mobile = major update driver
        + _diminishing(fw, max_effect=0.06, k=2.0)
    )

    # VSI: driven by infrastructure (primary) + operator training (secondary)
    delta_VSI = (
        _diminishing(infr, max_effect=0.15, k=2.0)  # more capacity = more volume
        + _diminishing(ot,   max_effect=0.05, k=2.5)  # better operators = more throughput
    )
...
def _gauge_chart(value: float, delta: float, title: str, color: str) -> go.Figure:
    ...
    fig = go.Figure(go.Indicator(
        ...
        value=simulated,
        delta={"reference": value, "valueformat": ".3f",
...
def _demand_forecast_chart(
    ...
    try:
        ...
        sim_80_hi = ci_80_hi * demand_multiplier
        ...
        fig.add_trace(go.Scatter(
            ...
            fill="toself", fillcolor="rgba(14,116,144,0.08)",
            line=dict(width=0), name="Baseline 95% CI", showlegend=True,
        ))
        fig.add_trace(go.Scatter(
            x=future_months + future_months[::-1],
            y=list(ci_80_hi) + list(ci_80_lo[::-1]),
            fill="toself", fillcolor="rgba(14,116,144,0.15)",
            line=dict(width=0), name="Baseline 80% CI", showlegend=True,
        ))

        # Baseline forecast
        fig.add_trace(go.Scatter(
            x=future_months, y=list(forecast_mean),
        ...
        # Simulated demand
        fig.add_trace(go.Scatter(
            x=future_months, y=list(sim_mean),
            mode="lines+markers", name=f"Simulated Demand (×{demand_multiplier:.2f})",
            line=dict(color=AMBER, width=3),
            marker=dict(size=6, color=AMBER),
        ))
        fig.add_trace(go.Scatter(
            x=future_months, y=list(sim_95_hi),
            mode="lines", name="Simulated 95% Upper",
            line=dict(color=AMBER, width=1.5, dash="dash"), opacity=0.7,
        ))
        ...
        hist_vals   = series[-12:]
        fig.add_trace(go.Scatter(
            x=hist_months, y=list(hist_vals),
        ...
        fig.update_layout(
            ...
            xaxis_title="Month", yaxis_title="Demographic Updates",
            legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
            height=380, margin=dict(l=40, r=20, t=50, b=40),
            paper_bgcolor=BG, plot_bgcolor="rgba(248,250,252,0.8)",
...
def render_simulator_panel(state_master: pd.DataFrame, state_month: pd.DataFrame) -> None:
    ...
    with viz_cols[1]:
        ...
        demand_fig = _demand_forecast_chart(state_month, selected_state, demand_multiplier)
